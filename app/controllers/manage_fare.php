<?php
class manage_fare extends Controller{
	public function __construct(){
		$this->postModel=$this->model('Manage_fares');
	}

	public function index(){
		$manage_fare=$this->postModel->get();
		$data = [
			'manage_fare'=>$manage_fare
		];
		$this->view('manage_fare/index', $data);
	}
	...
	public function create(){
		...
		if($_SERVER['REQUEST_METHOD']=='POST'){
            ...
            $numberValidation="/^[a-zA-Z0-9]*$/";
                ...
                $data['rateIDError']='Please Enter the Rate ID.';
                    ...
                    if($this->postModel->findRateByRateID($data['rateID'])){
			...
			if ($this->postModel->create_fare($data)) {
				header("Location: " . URLROOT . "/manage_fare");
			}else{
		...
		$this->view('manage_fare/create', $data);

	}

	public function edit($rateID){

		$manage_fare=$this->postModel->findFare($rateID);
		...
		if($_SERVER['REQUEST_METHOD']=='POST'){
            ...
            $numberValidation="/^[0-9]*$/";
                ...
                if(empty($data['fclassbase'])){
                    ...
                    //if moderatorID exists
                    if($this->postModel->findRateByRateID($data['fclassbase'])){
                ...
                if(empty($data['sclassbase'])){
                    ...
                    //if moderatorID exists
                    if($this->postModel->findRateByRateID($data['sclassbase'])){
                ...
                if(empty($data['tclassbase'])){
                    ...
                    //if moderatorID exists
                    if($this->postModel->findRateByRateID($data['tclassbase'])){
                ...
                if(empty($data['distance'])){
                    ...
                    //if moderatorID exists
                    if($this->postModel->findRateByRateID($data['distance'])){
                ...
                if(empty($data['rate'])){
                    ...
                    //if moderatorID exists
                    if($this->postModel->findRateByRateID($data['rate'])){
		...
		$this->view('manage_fare/edit', $data);
	}
		...
		$this->view('manage_fare/views', $data);
	}
	...
	public function delete($rateID){
		...
		if($_SERVER['REQUEST_METHOD']=='POST'){
			$_POST=filter_input_array(INPUT_POST, FILTER_SANITIZE_STRING);
		if($this->postModel->delete($rateID)){
			header("Location: " . URLROOT . "/manage_fare");
		}
		else{
			die('Something Going Wrong');
